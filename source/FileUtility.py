...
def load_file(file_name, spark_session):
    """
    Loads an indicated file after inspection into a spark session, only if file
    can be read

    Args:
        file_name: String of a fully qualified path to a file
        spark_session: A configured and established spark session

    Returns:
        Raw Spark dataframe, no schema inference of any types to avoid file re-scan. None if file is unreadable.
